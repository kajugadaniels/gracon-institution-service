...
Use this guide when setting up `api/institution` after cloning the project.

## Ownership

`api/institution` uses the shared database at runtime, but it does not own
...
```sql
CREATE ROLE gracon_institution_app LOGIN PASSWORD 'replace_with_institution_password';
```

If it already exists:

```sql
ALTER ROLE gracon_institution_app WITH PASSWORD 'replace_with_institution_password';
```
...
```env
DATABASE_URL=postgresql://gracon_institution_app:password@ep-example-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require
```
