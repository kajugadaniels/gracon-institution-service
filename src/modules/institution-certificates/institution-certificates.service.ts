...
export class InstitutionCertificatesService {
  ...
  async issue(
    ...
    if (!keyPair) {
      throw new BadRequestException(
        'Generate an institution key pair first at ' +
          'POST /institutions/:id/keys/generate.',
      );
    }
    ...
    if (existing && existing.notAfter > new Date()) {
      throw new ConflictException(
        'An active institution certificate already exists. ' +
          'Revoke it first or rotate the key pair to replace it.',
      );
    }
    ...
    // CRITICAL: discard immediately — key no longer needed
    privateKeyPem = null;
  ...
  async revoke(
    ...
    return {
      message: 'Institution certificate revoked. This action is permanent.',
      serialNumber: cert.serialNumber,
      revokedAt: new Date(),
    };
  }
