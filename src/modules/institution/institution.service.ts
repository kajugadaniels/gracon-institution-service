...
export class InstitutionService {
  ...
  async addMember(
    ...
    const existing = await this.prisma.institutionMember.findUnique({
  ...
  async grantStampAuthority(
    ...
    if (!resolution) {
      throw new NotFoundException(
        'Resolution not found or no longer active. Create or reactivate a resolution first.',
      );
    }
  ...
  // ─── Internal guards — used by every other module in this service ─────────

  /**
   * Throws 403 if the requesting user is not an active institution admin.
   * Called at the top of every institution-admin-only operation.
   */
  async assertInstitutionAdmin(
    institutionId: string,
    userId: string,
  ): Promise<void> {
    const institution = await this.prisma.institution.findUnique({
  ...
  /**
   * Validates the full authority chain for a stamp operation.
   * Called by api/stamp/ StampingService via shared DB reads —
   * this method is NOT called cross-service via HTTP.
   * Both services read the same Prisma tables.
   *
   * Note: This method is also exported for use by api/stamp/ which
   * duplicates the validation logic there against the same DB.
   * The stamp service must re-implement this check independently —
   * no cross-service HTTP dependency.
   */
