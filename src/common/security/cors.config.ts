/**
 * CORS configuration for the institution service.
 *
 * The institution API may be reached from multiple frontend origins (the
 * user app, the admin app, and the documents app via its proxy routes), so
 * we accept a comma-separated `FRONTEND_URLS` allowlist alongside the
 * primary `FRONTEND_URL`. Wildcards are never used.
 */
...
/**
 * Builds strict CORS config for the institution service.
...
export function buildCorsConfig(
  ...
  return {
    origin(origin, callback) {
      ...
      callback(
        new Error(
          `Origin ${origin} is not allowed by institution service CORS policy.`,
        ),
        false,
      );
    },
